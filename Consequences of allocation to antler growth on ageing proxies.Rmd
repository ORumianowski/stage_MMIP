...
!!!! LE  PARAGRAPHE SUIVANT DEVRAIT ETRE DANS LA PARTIE RESULTATS A MON AVIS, TU AS DECRIT LA METHODE DE STANDARDISATION, TU DECRIS DANS LA PARTIE RESULTATS CE QUE CA A DONNE. DE PLUS, QUAND TU METS UN GRAPHE TU LE DECRIS BIEN. 1) CE QU'ON REGARDE (DESCRIPTION DES AXES ET DES LEGENDES) 2) CE QUON CONSTATE DIFFERENTS MODELES 3) CE QU'ON CONCLUT.  !!!!!!!!!!!!!!!!
...
Four functions were proposed, depending on at most 3 unknown parameters, coefficients $\alpha$ and $\beta$ and the adult age, denoted $t_a$. !!! EXPLIQUE CE QU'EST LE HORVATH, POUR QUI C'ETAIT APPLIQUE !!!
\begin{align*}
f_1(Age) &= \alpha + \beta \times Age & \text{ Linear} \\
f_2(Age) &= \alpha \times \log(Age + 1) + \beta \times Age & \text{ Logarithmic}\\
f_3(Age) &= \left\lbrace 
\begin{array}{lr}
\beta \times \frac{Age-t_a}{t_a + 1} & \text{ if } Age \leq t_a\\
\alpha \times \log(t_a + 1) & \text{ if } Age > t_a
\end{array}
\right. & \text{ Horvath} \\
f_4(Age) &= \alpha \times Age + \beta \times Age^2 & \text{ Quadratic}
\end{align*}

and we used a tenfold cross validation to compare them. !!!! C'EST QUOI LA CROSS VALIDATION? EXPLIQUE!!!



!!! PAREIL, CES GRAPHIQUES DEVRAIENT ALLER DANS LA PARTIE RESULTATS. SI TU AS CHOISI DE FAIRE LE TEXTE EN FRANCAIS. ATTENTION DANS LA LEGENDE A METTRE LINEAR, METS DES MAJUSCULES AUX AXES, ET HARMONISE LOG (A GAUCHE) ET LOGARITHMIC (A DROITE). dE MEME DECRIT LES FIGURES !!!
The model with a simple logarithmic component was selected. The different functions	were: 
...
!!!! LA CA MANQUE CLAIREMENT DE CONTEXTE. TU VEUX MAINTENANT ANALYSER LES RESIDUS DU MODELE CHOISIT DANS LA SECTION PRECEDENTE. TU VEUX VOIR SI CES RESIDUS SONT STRUCTURES PAR LES VARIABLES EXPLICATIVES, DONC TU POSES UN MODELE LINEAIRE SUR LES RESIDUS. CE N'EST PAS TRIVIAL DONC EXPLIQUE LE, ET FORMALISE LE. ENTRAINE TOI A FORMALISER UN MODELE LINEAIRE (ICI LA NOTATION MATRICIELLE TE PERMETTRAIT DETRE CONCIS). 
DONC DANS L'ORDRE: 

- I) DESCRIPTION DU PROBLEME: VOIR SI UNE OU DES VARIABLES EXPLICATIVES STRUCTURENT LES RESIDUS, 
- II) PROPOSITION DE REPONSE: ESSAYER D'EXPLIQUER LES RESIDUS PAR LES VARIABLES EXPLICATIVES DANS UN MODELE LINEAIRE 
- III) DESCRIPTION DES VARIABLES EXPLICATIVES EMPLOYEES 
- IV) CHOIX DE MODELE PAR AICc (EXPLIQUE CE QUE C'EST) 
- V) ON L'A FAIT  EN R AVEC TELLE FONCTION DE TEL PACKAGE (EN VRAI, JE NE PENSE PAS QUE CA INTERESSE QUI QUE CE SOIT, MAIS POURQUOI PAS) 

ENSUITE, TU PARLES ICI EN 2 LIGNES DE LA RELATION ENTRE ANTLER LENGTH ET RTL. SOIT TU EN PARLES PAS DU TOUT, SOIT TU DEVELOPPES !!!

All statistical analyses were performed in the R environment version 4.0.3. The selection of linear mixed models based on the AICc !!! CEST QUOI L'AICc ? QUAND TU EMPLOIES UN TERME TECHNIQUE, DEFINIT LE!!! was carried out using the function `dredge` of the package `MuMin`.
For all analysis, antler length,  weight and population origin were used as explanatory variables. 
The cohort i.e. the birth date was added as a random effect. One individual with a null antler length was removed from the analysis. The antler length and the weight were log-transformed. All possible variable interactions were added (see Additional Information). Individuals were partitioned into four groups (1, 2-4, 5-7 and +7 years) to form the variable of age class. 
