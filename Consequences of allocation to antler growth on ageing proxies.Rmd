---
title: "Consequences of allocation to antler growth on epigenetic age deviation and telomere dynamics"
date: "16/05/2022"
output: 
  html_document:
    theme: simplex
...
J.F.L. conceived	the	ideas	 and	J.F.L. and P.G. designed	 methodology.	 O.R.	 analysed	 the data previously collected for another study, and wrote the manuscript. The paragraphs "Study populations", "methylation data" and part of "How to measure allocation to antler growth" written by J.F.L., were reused from a previous study.


# INTRODUCTION

Evolutionary theories of senescence, *i.e.* Antagonistic Pleiotropic Theory and Disposable Soma Theory, consider that aging emerges from the trade-off between reproduction in early life and in late life (see Lemaître et al. 2015 for a review). 
...
In the present study, we tested the relationship between the allocation to antler growth and these two proxies of aging *i.e.* telomere dynamics and epigenetic age deviation in roe deer (*Capreolus capreolus*), using a dataset from two French population measured in 2016 and 2017. 
...
We analysed the	relationship between antler length and	the	Julian date	of capture	(with	1	December	as day	0). We fitted	different	functions	to	assess	reliably	the	shape	of the relationship between	antler length	(log-transformed) and date	of	capture :	constant,	linear,	a	threshold	 function	 with	 one	 slope	 (i.e.	 antler length	 increasing linearly with	date of	capture until reaching a plateau at a	given	date)	and a	threshold	 function with two slopes (i.e. antler length increasing	linearly	with	date	of	capture	until	a	given	date and then	increasing further, but	with	a	different	slope).	Model	selection	was	based	on the Akaike	Information	Criterion	(AIC). The AIC criterion deals with the trade-off between the goodness of fit and the simplicity of the model: ${\displaystyle \mathrm {AIC} \,=\,2k-2\ln(L)}$, with $k$ the he number of estimated parameters and $L$ the maximum value of the likelihood function.

The models are tested on the data by discriminating according to the population of origin and whether the measurement was performed on a first year or not *i.e.* partitioning the dataset into four. Individuals with dry wood have been considered as fully grown, the raw measurement will be used directly as a standard measurement and these individuals were removed from the process of standardization.In order to retain as much data as possible, measurements of antlers where the antler condition is unknown were included in the standardization process.

## Analysis of epigenetic age deviation

### How to measure epigenetic age deviation

Because methylation dynamics are more active during development, epigenetic age does not evolve in a linear fashion. 
Following Horvath (2013), we computed the epigenetic deviation as the residuals of the models that best described the relationship between epigenetic age and chronological age.  

Formally, we suppose that there exists a $f$ such that:
$$\text{Methylated Age} = f(Age, \theta) + E,$$
where $f$ is a prediction function (potentially emerging from a physical model) depending on the age and on some unknown set of parameters $\theta$ that has to be estimated, and $E$ is a remainder that we call *residual*.

Four functions were proposed, depending on at most 3 unknown parameters, coefficients $\alpha$ and $\beta$ and the adult age, denoted $t_a$. 
\begin{align*}
f_1(Age) &= \alpha + \beta \times Age & \text{ Linear} \\
f_2(Age) &= \alpha \times \log(Age + 1) + \beta \times Age & \text{ Logarithmic}\\
f_3(Age) &= \alpha \times Age + \beta \times Age^2 & \text{ Quadratic}\\
f_4(Age) &= \left\lbrace 
\begin{array}{lr}
\alpha \times \log(Age + 1)& \text{ if } Age \leq t_a\\
\beta \times \frac{Age-t_a}{t_a + 1} + \alpha \times \log(t_a + 1)  & \text{ if } Age > t_a
\end{array}
\right. & \text{ Horvath} \\
\end{align*}

The three first functions were designed for the study. The fourth function (called “Horvath” hereafter) was used to developped a epigenetic age estimator for humans by Horvath (2013). This features of the function is that it is a continuous, monotonically increasing function which can be inverted, has a logarithmic dependence on age until adulthood and has a linear dependence on age after adulthood. To compare the four models, We used a tenfold cross validation. This method  consists of a resampling that uses different portions of the data to test and train a model. The metrics used here was the mean square error.  

### Is epigenetic age deviation explained by allocation to antler growth? 

#### Application of linear mixed models

The chosen link function  called "Logarithmic" provides the best prediction of epigenetic age from chronological age. Determining whether the residuals from this regression can be explained by biological variables would show that there is a biological signal contained in these residuals that is not explained by age. Linear mixed models(LMM) was used to test if there is an association between epigenetic age deviation $i.e.$ the residuals of the previous regression and antler growth, with the possibility to add covariates in order to control confounding effects. Moreover, random effect able to control the violation of the hypothesis of independance of two observations from a same category. Formally, the general model is : $$Y= X\theta + ZU + E$$ with $Y$ the response variable: the epigenetic age deviation $e.i.$ the residuals of the previous regression, $X\theta$ the fixed effects, $ZU$ the random effect and  $E$ the residuals.

The variance of each observation is: $$V(Y) = V(ZU) + V(E) = \gamma^2 + \sigma^2$$

The model to dependante observations: 

\begin{align*}
Cov(Yi,Yi') &= \left\lbrace 
\begin{array}{lr}
\gamma^2 & \text{ if } i = i'\\
\ 0  & \text{ if } i \neq \ i'
\end{array}
\right.  \\
\end{align*}
with $i$ the index the category associated to the random effect.

All statistical analyses were performed in the R environment version 4.0.3. The selection of linear mixed models based on the AICc was carried out using the function `dredge` of the package `MuMin`. AICc is AIC with a correction for small sample sizes: $AICc = AIC + \frac{2k^2+2k}{n-k-1}$ where n denotes the sample size and k denotes the number of parameters.


#### The tested models

For all analysis, antler length, weight and population origin were used as explanatory variables. The antler length and the weight were log-transformed. Individuals were partitioned into four groups (1, 2-4, 5-7 and +7 years) to form the variable of age class.The cohort i.e. the birth date was added as a random effect. One individual with a null antler length was removed from the analysis.  All possible variable interactions were added (see Additional Information). 

As an example, the common part of these models is: $$ Y_{ik}= \mu + \alpha \times AntlerLength_{ik} + \beta \times Weight_{ik} + U_i + E_{k} \  $$ with $\alpha$ the parameter associated with the effect of the antler length, $\beta$ the parameter associated with the effect of the weight, $U_i$ the random effect association to the cohort.

To test the relationship between antler length and the epigenetic age deviation, a first model was applied on all individuals with no lacking data (88 individuals). The age class was added as an explanatory variable. A second model  was applied only on the fawn (27 individuals). 
 

## Analysis of relative telomere length (RTL)

Similarly, the relationship between antler length and the RTL was tested. The response variable was replaced by the RTL. A first model was applied on all adults (123 individuals) and a second model only applied on the fawn. The relationship between antler length and the telomere dynamics was also tested thanks to the recaptures of 25 individuals in the second year making access to an annual RTL variation possible. 


# RESULTS 

## Standardisation of antler length

The AIC and graph analysis revealed that the linear model is the best fit (Figure 1), except for the group of adults from Chizé  (Figure 1.2) where the best model corresponds to the constant function. 
...
plan_experience = tibble(
  ma_fonction = c(constante, linear, one_slope, two_slopes),
...
ggplot(data_antler_2,
  ...
  geom_abline(slope = optim_output$par[1],
              intercept = optim_output$par[2],
              colour = "red")+
  labs( title = "Fig. 1.3:Trois-Fontaines - 1A",
        x= "Julian Capture Date",
        y= "Antler Length (log)")
...
ggplot(data_antler_2,
  ...
  geom_abline(slope = optim_output$par[1],
              intercept = optim_output$par[2],
              colour = "red")+
  labs( title = "Fig. 1.4: Trois-Fontaines - +1A",
        x= "Julian Capture Date",
        y= "Antler Length (log)")
...
## Measurements of Epigenetic Age Deviation

The cross-validation revealed that the second function called Logarithmic shows the best performance (Figure 2 & 3). Surprisingly, the Horvath function presents an intermediate performance. 
...
predicteur_linear <- function(parametre, Age){
  alpha = parametre[1]
  beta <- parametre[2]
...
cv_function = function(my_prop = 0.7, nb_replicates = 100){
  rerun(nb_replicates,# On fait tourner nb_replicates fois le meme traitement
        { 
          ...
          par_linear = optim(par = optimization_function(predicteur_linear,nb_par = 2), 
                                   predicteur = predicteur_linear, 
                                   my_data = train_data, 
                                   cost_function, 
          ...
          value_linear = cost_function(par_linear, predicteur_linear, my_data = test_data)
          ...
          tibble(method = c("Linear", "Logarithmic", "Quadratic", "Horvath"),
                 performance = c(value_linear, 
                                 value_Log,
                                 value_Quadratic,
                                 value_Horvath, ))
...
ggplot(data_antler) +
  aes(x = Age, y = DNAmAge) +
  geom_point()+
  stat_function(aes(color = "Linear"), 
                fun = predicteur_linear, 
                args = list(parametre  = optimization_function(predicteur_linear, 
                                                               nb_par = 2))) + 
  stat_function(aes(color = "Logarithmic"), 
                fun = predicteur_log, 
...
n = 1000
X = abs(rnorm(n))
Y = linear(c(2, 5, 1), X) + rnorm(length(X), sd = .5)
...
ggplot(data_antler,
  ...
  ggtitle("Figure 4: Relative Telomere Length  \n in relation to the Population")

```

# DISCUSSION

Male allocation to precopulatory sexual competition is assumed to explain male biais in intensity of senescence of mammals. In roe deer, demographic analysis suggested that high allocation to antler growth is associated with a lower probability to reach the age of 6. Nevertheless, our results have not revealed any significant association between allocation to antler growth and both proxies of aging i.e. epigenetic age acceleration and annual RTL variation. Therefore, we found no evidence that allocation to antler growth constitutes a physiological cost for roe deer. 

Epigenetic age deviation was not explained by any explanatory variables invoked in the study. For humans,  epigenetic clocks demonstrated its ability to predict mortality (Liu et al., 2020) and epigenetic age deviation proved to be sensitive to lifestyle factors including smoking behavior (Gibson et al., 2019). The absence of a relationship between allocation to antler growth and epigenetic age deviation suggests that production of antler does not constitute a physiological cost in terms of senescence. Nevertheless, the lack of association may be due to the method used in this study. The deviation index of epigenetic age is the residual of the regression of epigenetic age on chronological age. The link function chosen called "Logarithmic" provides the best prediction. Yet, it is not known if this logarithmic component has a biological explanation. A specific study is needed to explain the good prediction performance. Besides, it is still possible that the small size of the dataset was not sufficient to detect the associated cost. Moreover, the few recatures ($n=8$) suggested that the study of annual variation of epigenetic age deviation might be a more sensitive way to measure the effect of antler growth allocation on epigenetic age deviation. Data with more recaptures in order to quantify annual variation of epigenetic age deviation have to be carried out.

Surprisingly, our results suggest that RTL is not associated with age in roe deer. For a majority of individuals, telomere length increased with age (60%, $n=25$). While measurement error could contribute to such an apparent elongation of telomere length, increasing RTL might result from a possible expression of telomerase. Still, our knowledge on telomerase is limited and unclear (no expression in the fibroblast of Munjac species; expression in the antler of Sika deer). A significant difference of RTL was found between the two populations, individuals from Trois-Fontaines Forest having longer RTL. This result might be explained by the higher habitat quality of the forest (Salmón et al., 2016). 


### REFERENCES

Arneson, A., Haghani, A., Thompson, M. J., Pellegrini, M., Kwon, S. B., Vu, H. T., Li, C. Z., Lu, A. T., Barnes, B., Hansen, K. D., Zhou, W., Breeze, C.E., Ernst, J., & Horvath, S. (2021). A mammalian methylation array for profiling methylation levels at conserved sequences. BioRxiv. https://doi.org/10.1101/2021.01.07.425637
...
Delorme, D., Gaillard, J. M., & Jullien, J. M. (1988). Intérêt de l'étude de la période juvénile pour le suivi de l'évolution d’une population de chevreuils (Capreolus capreolus). Gibier Faune Sauvage, 5, 15–26.

Flerov, K. K. (1952). Fauna of USSR, Mammals Vol. 1, No. 2. Musk Deer and Deer. USSR Academia Science.

Gaillard, J., Delorme, D., Boutin, J. M., Van Laere, G., Boisaubert, B., & Pradel, R. (1993). Roe deer survival patterns: A comparative analysis of contrasting populations. Journal of Animal Ecology, 62, 778–791. https://doi.org/10.2307/5396
...
Gomes, N.M., Ryder, O.A., Houck, M.L., Charter, S.J., Walker, W., Forsyth, N.R., Austad, S.N., Venditti, C., Pagel, M., Shay, J.W., 2011. Comparative biology of mammalian telomeres: hypotheses on ancestral states and the roles of telomeres in longevity determination. Aging Cell 10, 761–768.
...
Kirkwood TB, Rose MR. 1991 Evolution of senescence: late survival sacrificed for reproduction. Phil. Trans. R. Soc. Lond. B 332, 15 –24. (doi:10. 1098/rstb.1991.0028)
...
Lemaître, J., Cheynel, L., Douhard, F., Bourgoin, G., Débias, F., Ferté, H., Gilot‐Fromont, E., Pardonnet, S., Pellerin, M., Rey, B., Vanpé, C., Hewison, A.J.M., Gaillard, J., 2018. The influence of early‐life allocation to antlers on male performance during adulthood: Evidence from contrasted populations of a large herbivore. J Anim Ecol 87, 921–932. https://doi.org/10.1111/1365-2656.12833
...
Liu, Z., Leung, D., Thrush, K., Zhao, W., Ratliff, S., Tanaka, T., Schmitz, L.L., Smith, J.A., Ferrucci, L., Levine, M.E., n.d. Underlying features of epigenetic aging clocks in vivo and in vitro 11.

Pettorelli, N., Gaillard, J.-M., Mysterud, A., Duncan, P., Chr. Stenseth, N., Delorme, D., Van Laere, G., Toïgo, C., & Klein, F. (2006). Using a proxy of plant productivity (NDVI) to find key periods for animal performance: The case of roe deer. Oikos, 112(3), 565–572. 
...
Seeker, L.A., Holland, R., Underwood, S., Fairlie, J., Psifidi, A., Ilska, J.J., Bagnall, A., Whitelaw, B., Coffey, M., Banos, G., 2016. Method specific calibration corrects for DNA extraction method effects on relative telomere length measurements by quantitative PCR. PLoS One 11, e0164046.

Salmón, P., Nilsson, J.F., Nord, A., Bensch, S., Isaksson, C., 2016. Urban environment shortens telomere length in nestling great tits, Parus major. Biol. Lett. 12, 20160155. https://doi.org/10.1098/rsbl.2016.0155

Sempéré,	 A.	J.,	&	Boissin,	 J.	(1981).	 Relationship	 between	 antler	 development	and	 plasma	 androgen	 concentrations	 in	adult	 roe	deer.	 Journal of Reproduction and Fertility,	 62,	49–53.	 https://doi.org/10.1530/ jrf.0.0620049
...
Sun, H., Zheng, W., Wu, S., Wang, C., Zheng, M., Tang, D., Zhu, Q., Zhang, Y., 2010. Detection of telomerase activity and expression of TERT gene in antler of sika deer. Anim. Husb. Feed Sci. 2, 12–20.
...
Ulm,	 K.,	&	Cox,	 C.	(1989).	 On	the	estimation	 of	the	threshold	 values.	 Biometrics,	45,	1324–1328.

Vanpé,	 C.,	Gaillard,	 J.	M.,	 Kjellander,	 P.,	Mysterud,	 A.,	Magnien,	 P.,	 Delorme,	 D.,	...	Hewison,	 A.	J.	M.	(2007).	 Antler	 size	 provides	 an	 honest	 signal	 of	male	 phenotypic	 quality	 in	roe	deer.	 The American Naturalist,	169,	481–493.	https://doi.org/10.1086/512046

Wilbourn, R.V., Froy, H., McManus, M.-C., Cheynel, L., Gaillard, J.-M., Gilot-Fromont, E., Regis, C., Rey, B., Pellerin, M., Lemaître, J.-F., 2017. Age-dependent associations between telomere length and environmental conditions in roe deer. Biol. Lett. 13, 20170434.

Williams GC. 1957 Pleiotropy, natural selection, and the evolution of senescence. Evolution 11, 398 –411. (doi:10.2307/2406060)

Zhou , W., Triche, T. J. Jr, L aird, P. W., & S hen , H . (2018). S e SAMe: Reducing artifactual detection of DNA methylation by Infinium BeadChips in genomic deletions. Nucleic Acids Research, 46(20), e123.
