...
Evolutionary theories of senescence, i.e. Antagonistic Pleiotropic Theory, Disposable Soma Theory, consider that aging emerges from the trade-off between reproduction in early life and in late life (see Lemaître et al. 2015 for a review). Therefore, because of sexual selection is associed with a diversity of cost (Garratt & Brooks, 2012), production and maintenance of secondary sexual traits (SST) is expected to correlate with senescence intensity. In mammals, the strenght of sexual selection negatively influenced the onset of senescence, and males of  polygynous species having an earlier onset of senescence than in monogamous species (Clutton-Brock & Isvaran 2007; Tidière et al., 2015). In an intraspecific perpective, sexual selection  might explained partially between-sex difference in senescence pattern. Males with a high investment in precopulatory behavior are expected to suffer a strengthened senescence. (Lemaître et al., 2014). Studies on  the cost associated with the development of SST have revealed diverse physiological costs (Garratt & Brooks, 2012). However, long-term costs were only unveiled from demographic studies (Lemaître et al., 2018). Very few studies have brought information able to determine if these long-term costs are physiological and what are the underlying mechanisms. 

A first approach to fill this gap is to know if the intensity of production and maintenance of SST affects proxies of aging. For example, in the Australian painted dragon (Ctenophorus pictus), males that maintained a high level of head coloration suffered from a rapid rate of telomere loss  Giraudeau et al., 2016). Although still highly discussed what is the relation between telomere dynamics and individual life history, their shortening correlates with increased mortality risk and is usually perceived as a good biomarker for explaining reproductive costs (Sudyka, 2019).  Based on the level of cytosine methylation at some selected CpG sites, highly accurate epigenetic age estimators recently developed offer another promising proxy to study the process of biological aging in ecology and evolution (Parrott and Bertucci, 2019). By interpreting the degree of epigenetic-to-chronological age discordance as a result of individual age acceleration, it can be used to quantify physiological long-term cost (Gibson et al. 2019).
...
All statistical analyses were performed in the R environment version 4.0.3. The selection of models was carried out using the function “dredge” of the package MuMin.  A p-value of 0.05 was considered significant. For all analysis, antler length,  weight, age, population origin and type of quality cohort were used as explicatories variatiables.
To test the relationship between antler length and the EAA, a first model was applied on all adults (X individuals) and a second model only applied on the fawn For all the analysis concerning EAA, the initial full model has the index of EAA as response variable. Similarly, the relationship between antler length and the RTL was tested with a first model applied on all adults (X individuals) and a second model only applied on the fawn. The relationship between antler length and the telomere dynamics was tested thanks to the recaptures of X individuals in the second year making access to an annual RTL variation possible. 


### RESULTS

#### Relationship between epigenetic age acceleration and allocation to antler growth

For all analyses, the constant model was selected.

#### Relationship between RTL and allocation to antler growth

For all analyses, the selected model contained the population origin as explanatory variable, with a difference of 0.43 (95% CI [-0.79 ; -0.06], P  =  0.021) between the two population, revealing the dependance of RTL to the population for roe deer.

#### Relationship between RTL annual variation and allocation to antler growth
